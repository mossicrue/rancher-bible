...
Let's add the stable repository as we want reproduce a _"production"_ environment
...
NOTE: For the course I followed the the [Install with Manifest](https://cert-manager.io/docs/installation/kubernetes/#installing-with-regular-manifests) guide.

TLDR: use one of these commands to install Cert Manager depending on your Kubernetes cluster version
```bash
# Kubernetes 1.16+
...
Check that cert-manager is working running
```bash
kubectl rollout status cert-manager -n cert-manager
kubectl rollout status cert-manager-webhook -n cert-manager
kubectl rollout status cert-manager-cainjector -n cert-manager
```
...
This is the default option when installing Rancher and it's the easyer one. It doesn't require any additional configuration, only specify the namespace and the hostname when installing.
